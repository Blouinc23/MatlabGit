function [wshft wms2 wms] = get_shifts(SpectrumReflectivity, verbose)
arguments
    SpectrumReflectivity double;
    verbose logical = true;
end
    if verbose
        wb = waitbar(0, "Finding shifts...");
    end

    art_data = load('ArtData.mat');
    %SpectrumReflectivity = art_data.SpectrumReflectivity;
    sumPV = squeeze(sum(10.^(SpectrumReflectivity/10), 1));
    SumPVLog = 10*log10(sumPV);
    ...
    nshft = size(SpectrumReflectivity, 1)-1;
    ...
    ITLOW=1;
    %ITHI=228;
    ITHI=size(SpectrumReflectivity, 3);
    %Zfact=3.0409;
    %For Middle Period
    %Zfact=10;
    ...
    for IT=ITLOW:ITHI
        if verbose
            waitbar(IT/ITHI, wb, sprintf("Finding MRR shifts.\nPeriod %d of %d...", IT, ITHI))
            %disp("Processing 10s period " + IT + " of " + ITHI + "...")
        end
        for k=1:128
            ...
            for i=1:nshft
                ...
                VarRtst(i,1)=var(rr, 'omitnan');
                MeanRtst(i,1)=mean(rr, 'omitnan');
                VarNtst(i,1)=var(nn, 'omitnan');
                MeanNtst(i,1)=mean(nn, 'omitnan');
    ...
    wshft(wshft == 1) = NaN;
    ZshftdB = 10*log10(Zshft);
    wshft(ZshftdB < 0.01) = NaN;

    wms2=-1*(wshft-32)*Delv;
if verbose
    waitbar(1, wb, 'Done!');
    pause(0.5)
    close(wb)
